...
virt-customize --no-network -a "$OUTPUT" \
  ...
  --run-command 'command -v update-initramfs >/dev/null 2>&1 || { echo "guest initramfs tooling is missing after offline package transaction" >&2; exit 1; }; command -v lsinitramfs >/dev/null 2>&1 || { echo "guest lsinitramfs tooling is missing after offline package transaction" >&2; exit 1; }; update-initramfs -u -k all; found=0; for initrd in /boot/initrd.img-*; do [ -e "$initrd" ] || continue; found=1; test -s "$initrd"; lsinitramfs "$initrd" | grep -qx init; lsinitramfs "$initrd" | grep -Eq "(^|/)(systemd-udevd|udevd)$"; done; [ "$found" -eq 1 ] || { echo "no guest initramfs image found after regeneration" >&2; exit 1; }' \
